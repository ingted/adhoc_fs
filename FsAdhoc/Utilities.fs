...
open Microsoft.FSharp.Reflection
    ...
    module Misc =
        ...
        let if' zero proc cond = if cond then proc() else zero
    ...
    module Seq =
        ...
        let tryNth n self =
            if n < 0 then
                None
            else    ((n, None), self) ||> Seq.fold (fun (n, result) x ->
    ...
    module Str =
        ...
        let private optFromIndex i = Option.if' (i >= 0) (fun () -> i)
        ...
        let sortAsCyclicOrder (cycle : 'T list) =
            ...
            let arcLengthFrom idx cntKinds (cycleBools : bool list) =
                let len, cntOccursion =
                    if not cycleBools.[idx] then    // 弧の最初が true でなければ最小ではありえないので無視
                        (0, 0)
                    else
                        (cycleBools |> Seq.rotate idx) |> Seq.fold (fun (len, cnt) b ->
                            if cnt = cntKinds
                                then (len, cnt)
                                else (len + 1, (if b then cnt + 1 else cnt))
                        ) (0, 0)
                if cntOccursion = cntKinds then Some len else None
    ...
    module Map =
        ...
        let keysIf pred map = map |> Map.filter pred |> Map.toSeq |> Seq.map fst
        let keys map = map |> keysIf (fun _ -> alwaysTrue)

        /// listof all values in the map, including duplications
        let valuesIf pred map = map |> Map.filter pred |> Map.toSeq |> Seq.map snd
        let values map = map |> valuesIf (fun _ -> alwaysTrue)
        let image map = values map
    ...
    type MultiSet<'T when 'T : comparison> internal(m : Map<'T, int>) (*as this*) =
        ...
        // constructors
        new() = new MultiSet<'T>(Map.empty)
        new(ms : MultiSet<'T>) = new MultiSet<'T>( (ms.ToMap() : Map<'T, int>) )
        ...
        member internal this.RepeatedSeq() =
            seq { for KeyValuePair (e, n) in m.ToArray() do
                    for i in 0..(n - 1) -> e }

        interface IEnumerable<KeyValuePair<'T, int>> with
            member this.GetEnumerator() = (this.ToMap() :> IEnumerable<KeyValuePair<'T, int>>).GetEnumerator()
            member this.GetEnumerator() = (this :> IEnumerable<KeyValuePair<'T, int>>).GetEnumerator() :> Collections.IEnumerator
