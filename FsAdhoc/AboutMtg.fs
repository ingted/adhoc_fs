...
module MTG =
    ...
    module Core =
        ...
        let allColorAtoms = [ White; Blue; Black; Red; Green ]
    ...
    module StrExp =
        ...
        type DUStr<'T>() =
            ...
            static member val Names =
                DUStr<'T>.Cases |> Array.map (fun (case : UnionCaseInfo) -> case.Name)
            static member FromString str =
                match DUStr<'T>.Cases |> Array.tryFind (fun case -> case.Name = str) with
        ...
        module Japanese =
            ...
            let jpCardTypes =
                [ "アーティファクト"; "クリーチャー"; "エンチャント"; "プレインズウォーカー"; "土地"; "インスタント"; "ソーサリー"; "部族" ]
...
module Scripts =
    ...
    module OriginalCards =
        ...
        module CardSyntaxParser =
            ...
            let colorAtomChar = (attempt colorAtomJpChar) <|> colorAtomEnChar
...
"""
        ...
        let colorsAndCMCFromManacostInMWS manacost =
            ...
            let pattern =
                patternlist
                |> List.map (fun (capture_name, pattern) -> "(?<" + capture_name + ">" + pattern + ")")
                |> Str.join "|"
            let matches = Regex.Matches(manacost, pattern, RegexOptions.ExplicitCapture)
        ...
        let spoilerCockatriceXmlFromText setShortName text =
            ...
            for card in cards do
                let colors, cmc = colorsAndCMCFromManacostInMWS (card.Cost)
                let colortags =
                    colors |> List.map (fun x -> "<color>" + x + "</color>") |> Str.join ""
                let typeline =
                    match card.TypeClass with
                    | (typ, Some cls) -> typ + " - " + (cls |> Str.trim)
                    | (typ, None) -> typ
                let tablerow = tableRowFromType (card.TypeClass |> fst)
                let pt =
                    match card.PowTou with
                    | Some (p, t) -> " <pt>" + p + "/" + t + "</pt>"
                    | None -> ""
                let text' =
        ...
        let downloadJapaneseCardImagesFromGatherer dir htmltext =
            ...
            let matches =
                let p ="""<a .*?href=".*?multiverseid=(\d+)"><img .*?alt="(.+?) \((.+?)\)" />"""
        ...
        let downloadCardNameTranslatorForJaFromWhisper setShortName =
            use webcl = new WebClient()
            let urlSpoilerText = "http://whisper.wisdom-guild.net/cardlist/" + setShortName + ".txt"
            let spoilerText = webcl.DownloadString (urlSpoilerText)
            let pattern = """英語名：(.+?)\n日本語名：(.+?)（"""
        ...
        let downloadJapaneseCardImagesFromCardGallery dir (htmltext: string) (translator: string -> string option) =
            ...
            let matches = Regex.Matches(htmltext, """<img .*?src="(.*?)" /><br /><i>(.+?)</i>""")
            let counter = ref 1
            use webcl = new WebClient()
            for m in matches do
                ...
                | Some urlImage, Some jaName ->
                    ...
                    | Some enName -> 
                        ...
                        webcl.DownloadFile (urlImage, filePath)
                    | _ -> printfn "translation error: \"%s\"" jaName
                | _ -> failwith "invalid html"
            ()
    ...
    let mainMtg (argv: string []) =
        // 画像ファイルのリネーム
        (*
        AboutMtg.Scripts.SpoilerProc.renamePics @"D:\Game\MWS\Pics\JOU"
        AboutMtg.Scripts.SpoilerProc.spoilerTextFromRss @"D:\Game\MWS\Pics\JOU"
        //*)
        ...
        let parsed = AboutMtg.Scripts.SpoilerProc.parseTextSpoiler spoilerText
        let cockxml = AboutMtg.Scripts.SpoilerProc.spoilerCockatriceXmlFromText "DVO" spoilerText
        //*)
        //do Scripts.SpoilerProc.downloadJapaneseCardImagesFromGatherer @"D:/Docs/downloads/M15" text
        (*
        do  let setShortName = "FRF"
            let dic = AboutMtg.Scripts.SpoilerProc.downloadCardNameTranslatorForJaFromWhisper setShortName
            AboutMtg.Scripts.SpoilerProc.downloadJapaneseCardImagesFromCardGallery (Path.Combine (@"D:\Game\MWS\Pics", setShortName)) text (fun s -> dic.TryFind s)
        //*)
        (*
        do  let setShortName = "M15"
            let dic = AboutMtg.Scripts.SpoilerProc.downloadCardNameTranslatorForJaFromWhisper setShortName
            Scripts.SpoilerProc.downloadJapaneseCardImagesFromUrlList (Path.Combine (@"D:\Game\MWS\Pics", setShortName)) text (fun s -> dic.TryFind s)
        //*)
